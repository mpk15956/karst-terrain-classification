...
-- Function to find a command path (cross-platform)
local function find_command(cmd)
  local is_windows = system.os == 'windows'
  
  -- Try Unix-style command check (works on Linux/Mac)
  ...
  -- Windows: check common locations and PATH
  if is_windows then
    -- Check PATH using where.exe (Windows)
    local handle = io.popen("where.exe " .. cmd .. " 2>nul")
    local result = handle:read("*a")
    handle:close()
    if result and result ~= "" then
      -- Parse first line, prefer .exe over .com
      local lines = {}
      for line in result:gmatch("[^\r\n]+") do
        table.insert(lines, line)
      end
      -- Prefer .exe files over .com files
      for _, line in ipairs(lines) do
        if line:match("%.exe$") then
          return line:match("^%s*(.-)%s*$")
        end
      end
      -- If no .exe found, return first line
      if #lines > 0 then
        return lines[1]:match("^%s*(.-)%s*$")
      end
    end
    
    -- Check common Windows installation paths for pdflatex
    if cmd == "pdflatex" then
      local common_paths = {
        "C:\\texlive\\2025\\bin\\windows\\pdflatex.exe",
        "C:\\texlive\\2024\\bin\\windows\\pdflatex.exe",
        "C:\\texlive\\2023\\bin\\windows\\pdflatex.exe",
        os.getenv("TEXLIVE_ROOT") and pandoc.path.join({os.getenv("TEXLIVE_ROOT"), "bin", "windows", "pdflatex.exe"}) or nil,
    ...
    if cmd == "inkscape" then
      local common_paths = {
        ...
        os.getenv("LOCALAPPDATA") and pandoc.path.join({os.getenv("LOCALAPPDATA"), "Programs", "Inkscape", "bin", "inkscape.exe"}) or nil
      }
      for _, path in ipairs(common_paths) do
        if path then
          local file = io.open(path, "r")
...
-- Function to check if a command exists (backwards compatibility)
local function check_dependency(cmd)
  return find_command(cmd) ~= nil
end
...
-- Supports both Quarto-style (#|) and LaTeX-style (%%|) comments
local function properties_from_code (code, comment_start)
  local props = {}
  -- Try Quarto-style first (#|), then LaTeX-style (%%|)
  local patterns = {
    '#| ?([-_%w]+): ([^\n]*)\n',  -- Quarto style: #| key: value
    comment_start:gsub('%p', '%%%1') .. '| ?([-_%w]+): ([^\n]*)\n'  -- LaTeX style: %%| key: value
  }
  ...
  for _, pattern in ipairs(patterns) do
    for key, value in code:gmatch(pattern) do
      if key == 'fig-attr' then
        ...
        local subpatterns = {
          '#|   ([^\n]+)\n',  -- Quarto style
          comment_start:gsub('%p', '%%%1') .. '|   ([^\n]+)\n'  -- LaTeX style
        }
        for _, subpattern in ipairs(subpatterns) do
...
local function compile_tikz_to_svg(code, user_opts, conf, basename)  -- Added conf and basename parameters
  -- Find required dependencies and get their full paths
  local pdflatex_path = find_command('pdflatex')
  local inkscape_path = find_command('inkscape')
  
  -- Fallback: Use known paths if where.exe didn't work (common on Windows)
  -- Always try fallback on Windows since where.exe parsing can be unreliable
  local is_windows = (system.os == 'windows' or package.config:sub(1,1) == '\\')
  if (not pdflatex_path or pdflatex_path == "") then
    if is_windows then
      -- Try common TeX Live paths in order of likelihood
      pdflatex_path = "C:\\texlive\\2025\\bin\\windows\\pdflatex.exe"
    end
  end
  
  if (not inkscape_path or inkscape_path == "") then
    if is_windows then
      -- Try common Inkscape paths
  ...
  if not inkscape_path or inkscape_path == "" then
    error("Inkscape not found. Please install Inkscape to convert PDFs to SVG, " ..
          "or add it to your PATH if it's already installed.")
  end
...
\end{document}
      ...
      local success_inkscape, inkscape_result = pcall(pandoc.pipe, inkscape_path, args, '')
...
-- Function to strip Quarto code block options (#| lines) from code
local function strip_quarto_options(code)
  local lines = {}
  for line in code:gmatch("[^\r\n]+") do
    -- Skip lines that start with #| (Quarto code block options)
...
local function code_to_figure(conf)
  return function(block)
    ...
    -- Check for both 'tikz' and '{tikz}' class (Quarto may preserve braces)
    local is_tikz = block.classes:includes('tikz') or 
                    block.classes:includes('{tikz}')
    if not is_tikz then
      return nil
    end
    
    -- Debug: Log that we found a TikZ block (only in verbose mode)
    if quarto and quarto.log then
      quarto.log.info("Processing TikZ code block...")
    end

    -- Strip Quarto options (#| lines) from the code before processing
    local clean_code = strip_quarto_options(block.text)
    
    -- Get options from code block (this parses #| lines)
    local dgr_opt = diagram_options(block)

    -- Get basename for file naming (use clean code for hash)
    local basename = dgr_opt.filename or pandoc.sha1(clean_code)

    -- Check if image is cached (use clean code for hash)
    local hash = clean_code
    ...
    if not imgdata or not imgtype then
      -- No cached image; compile TikZ code (use clean code without #| lines)
      local success, result = pcall(function()
        return compile_tikz_to_svg(clean_code, dgr_opt.opt, conf, basename) -- Pass conf and basename
      end)
      if not success then
