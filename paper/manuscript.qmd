...
![From physical terrain to drainage topology on a real GLO-30 patch: (a) hillshade, (b) D8 flow accumulation, (c) channel network at the matched NHD density, (d) the H0 persistence diagram of the donor-graph merge tree. A physical ridgeline dictates the topological split.](figures/fig-pipeline.png){#fig-pipeline width=100%}
...
![Spatial-null MMD$^2$ p95 floor versus sample size; the by-tile null tightens monotonically and the K=10 operating point (about 113 patches per side) is fixed before generation.](figures/fig-power.png){#fig-power width=70%}
...
![The 20 one-degree study tiles colored by physiographic province; labels give the NHD drainage-density quintile.](figures/fig-studyarea.png){#fig-studyarea width=75%}
...
![Null (real vs real) versus test (generated vs real) MMD$^2$ at the K=10 operating point; the test median sits at 3.12x the p95 floor and no null split exceeds it.](figures/fig-headline.png){#fig-headline width=60%}

![Looks fine, routes wrong: a real GLO-30 patch and a MESA-generated patch of comparable appearance, with visibly different channel networks and H0 persistence diagrams.](figures/fig-qualitative.png){#fig-qualitative width=90%}
...
![Optical test/floor ratios (RBF-MMD and KID) for CLIP and Inception, with the province-swap positive control and the topology reference at 3.12x; every optical metric separates generated from real.](figures/fig-optical.png){#fig-optical width=80%}
...
![Sensitivity profile in matched MMD$^2$/floor currency (trench, phase-noise, MESA) and the saddle-flip caveat; MESA is the largest response and no upper bound is established.](figures/fig-sensitivity.png){#fig-sensitivity width=95%}
